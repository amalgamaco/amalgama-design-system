...
const SearchViewResult = React.forwardRef<HTMLLIElement, SearchViewResultProps>(
  ({ className, icon, children, ...props }, ref) => (
    <li
      ref={ref}
      className={cn(
        "flex items-center gap-3 px-4 py-3 cursor-pointer text-on-surface text-body-md transition-colors duration-fast ease-default hover:bg-[var(--color-nav-hover)] first:rounded-t-[27px] last:rounded-b-[27px]",
        className
      )}
      {...props}
    >
