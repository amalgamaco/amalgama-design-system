...
- **Short forms → `Dialog`.** A focused task of a few fields (renombrar, invitar, cambio rápido de estado) belongs in a `Dialog`, keeping the user in context. Put the primary action as a filled `Button variant="primary"` and the cancel as `variant="tertiary"`.
