...
| Calendar / Date Picker | `calendar.css`/`date-picker.css` — **dynamic** `Date`-math month renderer (`initCalendar()` on `[data-calendar]`); own **Calendar** page (`c-calendar`, Overview=Basic) + Date Picker page. Patterns: month/year dropdown caption (`data-cal-caption="dropdown"`), range (`data-cal-mode="range"`), **presets** (`.calendar-with-presets` + `calApplyPreset`), **date+time** (`.calendar-time-row` + `calTimeSync`), **booked dates** (`data-cal-booked`, non-selectable), **custom cell size** (`--cal-cell` / `.calendar-sm`/`-lg`), **week numbers** (`data-cal-weeknumbers`, ISO-8601). `Intl.DateTimeFormat` labels (no locale lib), absolute popover (no collision detection), no roving-grid keyboard (vanilla vs react-day-picker) | `calendar.tsx`, `date-picker.tsx` | `date-picker.html` |
