...
**Drawer added (2026-07, `drawer.tsx`):** the `vaul`-based, gesture-driven panel — official shadcn Drawer structure, Embassy tokens (surface/scrim/heading/label), 4 directions + drag handle on the bottom variant. It is the **touch/mobile** counterpart to **Sheet** (Radix Dialog side panel for desktop); both coexist by design (shadcn's own Sheet-vs-Drawer split) — see GOVERNANCE §19.1. Full docs page (Overview/Specs/Guidelines/Accessibility/Code) + island (`drawer-showcase`/`drawer-bottom-showcase`) + `drawer-tokens` panel + nav under the Sheets group. Dep: `vaul`. **Only remaining shadcn non-addition: `sidebar` → use the app-shell (§19.2).** Official coverage now 45/46 (hover-card consolidated into `rich-tooltip`).
