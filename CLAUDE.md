...
| Command / Combobox | `command.css`/`combobox.css` — substring filtering (not fuzzy) + arrow-key/Enter nav + empty state via `initCommand()`; **⌘K Command Dialog** (`.command-dialog`) = Command inside a scrim overlay | `command.tsx`, `combobox.tsx` | — (no standalone doc page yet) |
...
| Calendar / Date Picker | `calendar.css`/`date-picker.css` — **dynamic** `Date`-math month renderer (`initCalendar()` on `[data-calendar]`) with real month nav + **month/year dropdown caption** (`data-cal-caption="dropdown"`); `Intl.DateTimeFormat` labels (no locale library), absolute-positioned popover (no collision detection); basic range selection | `calendar.tsx`, `date-picker.tsx` | `date-picker.html` |
